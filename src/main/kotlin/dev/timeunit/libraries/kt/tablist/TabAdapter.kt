...
class TabAdapter :
    PacketAdapter(_root_ide_package_.dev.timeunit.libraries.Library.getInstance(), PacketType.Play.Server.PLAYER_INFO, PacketType.Play.Server.NAMED_ENTITY_SPAWN) {

    override fun onPacketSending(event: PacketEvent) {
        if (_root_ide_package_.dev.timeunit.libraries.Library.getInstance().tabEngine.layoutProvider == null || !this.shouldForbid(event.player)) {
            return
        }
        ...
        if (event.packetType === PacketType.Play.Server.PLAYER_INFO) {
            ...
            Bukkit.getScheduler().runTask(_root_ide_package_.dev.timeunit.libraries.Library.getInstance()) {
    ...
    private fun shouldForbid(player: Player): Boolean {
        ...
        val playerTab = _root_ide_package_.dev.timeunit.libraries.Library.getInstance().tabEngine.tabs[playerName]
