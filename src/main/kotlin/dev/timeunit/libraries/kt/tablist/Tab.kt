...
class Tab(private val player: Player) {
    ...
    fun update() {
        if (_root_ide_package_.dev.timeunit.libraries.Library.getInstance().tabEngine.layoutProvider != null) {
            val tabLayout = _root_ide_package_.dev.timeunit.libraries.Library.getInstance().tabEngine.layoutProvider?.provide(this.player)
            if (tabLayout == null) {
                reset()
                return
            }
